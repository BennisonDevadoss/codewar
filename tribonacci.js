...
Test.assertSimilar(tribonacci([1,0,0],10),[1,0,0,1,1,2,4,7,13,24])  sum of last three element. 
 */

function tribonacci(signature, n) {
    let total = signature.reduce((a, b) => {
        return a + b
    })
    let len = signature.length;

    signature.push(total);
    if (n > (signature.length - 1)) {
        for (i = 0; i < n - len -1; i++) {
        ...
        // let arr = signature.filter(a => signature.indexOf(a) < n);
        // console.log("arr", arr)
        // return arr;
        let arr = [];
        for (i = 0; i < n; i++) {
